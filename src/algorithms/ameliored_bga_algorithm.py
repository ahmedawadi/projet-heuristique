...
def ameliored_genetic_algorithm(N, D, D1, f, i, result_folder_path, Delta):
    ...
    with ThreadPoolExecutor() as executor:
        fitnesses = np.array(list(executor.map(f, parents))) # Evaluate fitness in parallel
    ...
    while t < MAX_ITER:
        ...
        with ThreadPoolExecutor() as executor:
            enfants_fitnesses = np.array(list(executor.map(f, enfants)))  # Evaluate fitness in parallel
...
def run_ameliored_genetic_algorithm_executions(Delta, result_folder_path):
    for i in range(10, 11):
        if i < 7:
            for _ in range(0, 30):
                ameliored_genetic_algorithm(
